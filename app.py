...
# --- Page Configuration ---
st.set_page_config(page_title="Pro-Tracer v4.6", layout="wide")
...
st.title("🛡️ Pro-Tracer: Stable UI Edition")
...
if df_raw.empty:
    ...
    if mode == "Global 3D Optimizer":
        st.session_state.run_audit = False # Clear audit state when switching modes
        st.header("🔬 3D Precision Scan")
        # ... (Optimizer code remains the same as v4.5)
        if st.button("🚀 Start Power-Level Scan"):
            # (Optimizer Logic)
            st.write("Scan results would appear here...")

    # --- MODULE 2: TRADE DETAILER (FIXED) ---
        ...
        # Use on_click to persist the state
        st.button("📊 Run Detailed Audit", on_click=trigger_audit)
        ...
        if st.session_state.run_audit:
            ...
            if trades:
                ...
                st.warning("No trades found. Try lowering the RSI Entry or Volume Spike.")